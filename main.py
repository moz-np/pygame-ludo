...
def main():
    ...
    bg1 = pygame.image.load('icons/menubg1.jpg')
    ...
    playerselect = pygame.transform.scale(pygame.image.load(os.path.join('icons', 'message2.png')), (450, 50))

    while True:
        back=False
        screen.blit(bg1,(0,0))
        screen.blit(play,(10,10))
        screen.blit(rules,(10,110))
        screen.blit(quit_,(10,210))
        mx, my = pygame.mouse.get_pos()
        if (mx>=10 and mx<=130) and (my>=10 and my<=60):
            selected(screen,[10,10,120,50])
        elif (mx >= 10 and mx <= 130) and (my >= 110 and my <= 160):
            selected(screen,[10, 110, 120, 50])
        elif (mx >= 10 and mx <= 130) and (my >= 210 and my <= 260):
            selected(screen,[10, 210, 120, 50])
        for event in pygame.event.get():
            if event.type==pygame.MOUSEBUTTONDOWN:
                if (mx >= 10 and mx <= 130) and (my >= 10 and my <= 60):
                    screen.blit(bg2, (0, 0))
                    screen.blit(img1, (20, 20))
                    screen.blit(img2, (360, 20))
                    screen.blit(opt1,(20,230))
                    screen.blit(opt2,(360,230))
                    pygame.display.update()
                    while True:
                        ...
                        for event2 in pygame.event.get():
                            if event2.type == pygame.QUIT:
                                ...
                                if (newmx >= 20 and newmx <= 220) and (newmy >= 20 and newmy <= 220):
                                    ludoGame.n=ludoPrompt(screen,bg2,gattiselect)
                                    if ludoGame.n==0:
                                        back=True
                                        break
                                    else:
                                        ludoGame.players=ludoPrompt(screen,bg2,playerselect)
                                        ludoGame.main()
